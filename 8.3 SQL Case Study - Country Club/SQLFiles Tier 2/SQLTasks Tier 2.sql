...
SELECT name AS charged_facilities
FROM Facilities
WHERE membercost == 0;

/* Q2: How many facilities do not charge a fee to members? */
SELECT COUNT(facid) AS number_of_member_free_facilities 
FROM Facilities
WHERE membercost == 0;
...
SELECT name,
    CASE WHEN monthlymaintenance > 100 THEN "expensive"
        ...
        ELSE NULL END AS monthlymaintenance
FROM Facilities;
...
the member name. */
SELECT DISTINCT Facilities.name AS tennis_court,
...
FROM (SELECT Facilities.name AS facility,
    ...
    INNER JOIN Members
        ON Bookings.memid = Members.memid
    WHERE date(starttime) == '2012-09-14' AND cost > 30) AS subquery
ORDER BY cost;
...
SELECT surname || ", " || firstname AS member_name,
    s1.member_name AS recommendedby_name
FROM Members
LEFT JOIN s1
    ON Members.recommendedby = s1.memid
